...
def initialize(p_sovitsPath, p_sovitsV3Path, p_sovitsV4Path, p_gptPath, p_defaultReferPath, p_defaultReferText, p_defaultReferLanguage, p_device, p_halfPrecision, p_streamMode, p_mediaType, p_subType, p_cnhubertBasePath, p_bertPath, p_bigvganPath, p_vocoderPath, p_svPath, p_g2pwPath):
    global sovits_path
    global path_sovits_v3
    global path_sovits_v4
    global gpt_path
    global default_refer_path
    global default_refer_text
    global default_refer_language
    global device
    ...
    path_sovits_v4 = p_sovitsV4Path
    gpt_path = p_gptPath
    default_refer_path = p_defaultReferPath
    default_refer_text = p_defaultReferText
    default_refer_language = p_defaultReferLanguage
    device = p_device
    full_precision = not p_halfPrecision
    half_precision = p_halfPrecision
    stream_mode = p_streamMode
    media_type = p_mediaType
    sub_type = p_subType
    cnhubert_base_path = p_cnhubertBasePath
    bert_path = p_bertPath
    bigvgan_path = p_bigvganPath
    vocoder_path = p_vocoderPath
    sv_path = p_svPath
    g2pw_path = p_g2pwPath
...
async def infer(
    ...
    if not isInitialized:
        initialize(sovits_path, sovits_v3_path, sovits_v4_path, gpt_path, refer_wav_path, prompt_text, prompt_language, device, half_precision, stream_mode, media_type, sub_type, cnhubert_base_path, bert_path, bigvgan_path, vocoder_path, sv_path, g2pw_path)
        isInitialized = True
    ...
    with open(output_path, 'wb') as f:
        async for audio_data in body_iterator:
            f.write(audio_data)
